...
HardwareSerial Serial3(PB11, PB10);   // RX , TX UART 2
// HardwareSerial Serial1(PA_10, PA_9); For UART 1
...
if (runEnable) {
  ...
  switch (sysState)
  {
    ...
    case ST_IDLE:
      ...
      resetFanPrerun();
      if (!runEnable) {
  resetFaultLatch();   // ✅ acknowledge faults
}
    ...
    case ST_WAIT_SET:
      ...
      if (askSent && millis() - waitSetStart >= ASK_RETRY_MS) {
        ...
        if (!eepromFallbackUsed) {
          ...
          Serial3.println("{\"ACK\":\"SET\",\"SRC\":\"EEPROM\"}");
        }
